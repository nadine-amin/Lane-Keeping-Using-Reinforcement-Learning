...
class Driver(object):
    ...
    def init(self):
        #Returns init String With Rangefinder Angles
    ...
    def drive(self, msg):
        ...
        trackpos = self.state.getTrackPos() #Stores The TrackPos Value
        angle    = self.state.getAngle()    #Stores The Angle Value
              
        state=GetState2.State(speed,track,trackpos) #Gets The State in The Partitioned Form
        ...
        if start==0:
            ...
            Qtable.update(Current_State,Taken_ActionIndex,Qmax_Next,Next_State,Reward_Next,self.table)  #Updates The Q-Table
            
            Current_State = Next_State
            Taken_ActionIndex = Next_Taken_ActionIndex
                     
        if Reward_Next==-2 or self.state.getDamage() > 8500 :
            #Set Meta to Trigger Restart
